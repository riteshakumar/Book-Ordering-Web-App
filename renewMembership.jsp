...
<body>
       <h1>Library Management System </h1>
        ...
        <br></br>
    <div class="container">
        <form class="form-inline" method="post" action="saveRenew.jsp">
            
        <label for="memid">Member</label>	
        <select id="memid" name="memid" class="js-example-basic-single1" required>
            <option value=""></option>
            <%
                    Statement statement;
                    ResultSet r;
                    String q = "select m.*,c.*,library_pkg.member_full_name_c(c.cd_num) full_name from MEMBER m, CARDS c where m.cd_num = c.cd_num and c.expiry_dt<sysdate";
                    ...
                    while(r.next())
                    {
                        %>
                            ...
                            <option value='<%= r.getString(9)%>'> <%= r.getString(15)%></option>
                        <% 
                    }
              %>
        </select>
        <br><br>
    <label for="yrs">Renew for Years</label>	
    <input type="number" id="yrs" placeholder="Enter Years" name="yrs" required>
